...
public function listarProductos(){
		...
		$sql="SELECT PRO.id,PRO.imagen,PRO.es_destacado,PRO.modelo,PRO.tiene_descuento,PRO.precio
			 FROM producto PRO 
		...
		if($fila['es_destacado']==1){
			...
			if($fila['tiene_descuento']==0){
					...
					<div class='producto-responsive text-center'>
							...
							<img  src=img/".$fila['imagen']." width='200' height='200' alt='Oeste Neumaticos - GBA - Zona Oeste - Ituzaingo  ".$fila['modelo']."'>
							<h3>".strtoupper($fila['modelo'])."</h3>
	...
	public function listarProductosHombres(){
		...
		$sql="SELECT PRO.id,PRO.imagen,PRO.es_destacado,PRO.modelo,PRO.tiene_descuento,PRO.precio FROM producto PRO 
...
public function listarUnSoloProducto($id){
		...
		while($fila=$resultado->fetch_assoc()){
			...
			"	
				...
				<div class='row'>
						<select class='form-control select-talle' name='talle' id='talle' style='margin-bottom:1%;' id='select-talle'>
				...
				<div class='col-lg-5 col-md-12 col-sm-12 col-xs-12 ver-mas-col'>
					<div class='img-gal-cont'>
						 <a data-fancybox='gallery' href='img-md/".$fila['imagen_descripcion_lg']."' class='image-cont'>
					   	...
					   	<img src='elementos_separados/zoom-icon.png' width='15px'height='15px'style='position: absolute;left:0; top: 10px;z-index: 10;' />
					...
					<div class='row'>
						...
						<div class='img-gal-cont'>
							...
							<a data-fancybox='gallery' href='img-md/".$fila['imagen_descripcion_2']."' class='image-cont'>
						   	...
						   	<img src='elementos_separados/zoom-icon.png' width='15px'height='15px'style='position: absolute;left:141px; top: 10px;z-index: 10;' />
						...
						<div class='img-gal-cont'>
							<a data-fancybox='gallery' href='img-md/".$fila['imagen_descripcion_6']."' class='image-cont'>
						   	...
						   	<img src='elementos_separados/zoom-icon.png' width='15px'height='15px'style='position: absolute;left:259px; top: 10px;z-index: 10;' />
					...
					<div class='row'>
						<div class='img-gal-cont'>
							<a data-fancybox='gallery' href='img-md/".$fila['imagen_descripcion_4']."' class='image-cont'>
						   	...
						   	<img src='elementos_separados/zoom-icon.png' width='15px'height='15px'style='position: absolute;left:2px; top: 10px;z-index: 10;' />
			...
			"
					...
					<div class='row'>
						<div class='container'>
							...
							<div class='hidden-lg hidden-md col-sm-4 hidden-xs'>
								<img src='img-md/".$fila['imagen_descripcion_5']."'  width='150px' height='220px' >
							</div>
						</div>
					</div>	
		...
		$idProd=0;
			...
			if($fila_buscar_venta['comprobante_nro']==''){
					...
					if ($producto->cantidad!=0) {
						...
						$stmt=$this->mysqli->prepare("INSERT INTO usuario_compra_producto(id_mp,comprobante_nro,email_usuario,id_producto,fecha,cantidad,marca,modelo,precio,tiene_descuento,precio_descuento,recibio_email,id_medio_pago,id_estado_envio,id_estado_venta)
					  		  					VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,2,3)");

						$stmt->bind_param("issisissdidii",$id_mp,$referencia,$email,$producto->id,$fecha,$producto->cantidad,$producto->marca,$producto->modelo,$producto->precio,$producto->tiene_descuento,$producto->precio_descuento,$recibio_email,$medio_de_pago);
...
public function buscarCiudadSegunProvincia($provincia){
		...
		$stmt=$this->mysqli->prepare("SELECT CIU.ciudad_nombre	
		  FROM ciudad CIU JOIN provincia PRO ON CIU.provincia_id=PRO.id  
		  WHERE provincia_nombre like ? AND CIU.id!=0
